...
app.post('/addUser', cors(corsOptions), (req, res, next) => {
	const { data } = req.body;
	const {pass, email, name } = data;
	//do basic validation here
	connection.query(`INSERT INTO Users(email, name, password) VALUES ('${pass}', '${email}', '${name}')`, (err, res, field) => {
		if (eb) throw err;
...
request(options, function (error, response) {
  ...
  for(i in result){
	...
	console.log(country) 
		var sql = `INSERT INTO Launches(location_name, country, longitude, latitude, utc_offset) VALUES ('${name}','${country.code}','${longitude}','${latitude}','${utc_offset}')`
